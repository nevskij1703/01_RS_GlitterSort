...
|---|---|---|---|
| **Interstitial** | Между уровнями — каждые 2 уровня, пройденных БЕЗ рекламы (не раньше L3) | `Ads.showInterstitial()` | `Ads.UNIT_INTERSTITIAL` |
| **Rewarded** | По клику «+ Колба» в bottom bar и в попапе «Недостаточно молний» | `Ads.showRewarded()` | `Ads.UNIT_REWARDED` |

### Частота межстраничной рекламы

Ключевая величина — `adFreeStreak` в сейве: сколько уровней подряд игрок прошёл, **не увидев ни одного ролика** (ни rewarded за колбу, ни rewarded за молнии, ни интерстишл).

| Событие | Что происходит со счётчиком |
|---|---|
| уровень пройден без рекламы | `+1` |
| уровень пройден с рекламой | не меняется, и интерстишл после такого уровня не показываем |
| показали интерстишл | `= 0` |

Показ, когда одновременно: формат включён (`Ads.INTERSTITIAL_ENABLED`), уровень ≥ `INTERSTITIAL_MIN_LEVEL` (3), на уровне не было рекламы, счётчик ≥ `ADFREE_STREAK_FOR_INTERSTITIAL` (2) и прошло ≥ 2 мин с прошлого показа.

Как это видит игрок:

- никогда не смотрит ролики → реклама после L3, L5, L7, L9…;
- посмотрел ролик на уровне → после него тишина, показ сдвигается на следующий «тихий» уровень;
- активно смотрит rewarded → межстраничную практически не видит (это и есть замысел: формат не наказывает тех, кто и так приносит доход).

Два предохранителя, которых нет в ТЗ:

- **cooldown 2 мин** (в памяти, сбрасывается перезапуском). Если показ подавлен кулдауном, счётчик **не** сбрасывается — реклама просто сдвигается на следующий уровень. На быстрых уровнях это заметно: два уровня можно пройти быстрее двух минут.
- **recovery 5 мин**: игрок закрыл приложение внутри рекламы → на следующем запуске показ доигрывается (`interstitialPendingAt` в сейве), и только потом грузится уровень.

`INTERSTITIAL_ENABLED = false` остаётся аварийным тумблером: гасит показ и прогрев целиком, `shouldShowInterstitialNow` возвращает `false`, recovery не запускается, событие `ad_interstitial_shown` не отправляется.
