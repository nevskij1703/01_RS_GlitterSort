...
**Часть чисел игры больше не только в коде.** Ритм межстраничной
(`interstitial_min_level`, `interstitial_adfree_streak`,
`interstitial_cooldown_sec`), порог просьбы об оценке и вся экономика молний
(`energy_per_level`, `energy_reward`, `energy_regen_cap`,
`energy_regen_minutes`) читаются через `tuned()` в блоке `[APP]`. Значение
приезжает из файла в облаке и меняется **без выпуска обновления**.
...
- Читать значения — только через `tuned(ключ, константа)` и **при обращении**, а
  не в константу при старте: иначе значение застынет и правка в облаке не
