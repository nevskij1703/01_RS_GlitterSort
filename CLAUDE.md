...
## Обновление изнутри игры: RuStore In-App Updates

Флаг `-RuStoreUpdateSdk` (из `"rustoreUpdateSdk": true`) + JS-обёртка — inline
IIFE `window.RuStoreUpdateClient`, блок `[RUSTORE_UPDATE]` в
[index.html](index.html) сразу после `RuStoreReviewClient`. Контракт
(`window.RuStoreUpdate.check/start/complete`, колбэки `__rustoreUpdateInfo` /
`__rustoreUpdateResult` / `__rustoreUpdateState`) зашит в
`RuStoreUpdateBridge.java`.

**Зачем вообще.** Поставивший игру к карточке в магазине больше не
возвращается: у него на экране ярлык, а не витрина. Без этого SDK он остаётся на
скачанной версии со всеми её ошибками, и починить их для него нельзя никак.

**Сценарий `flexible`**: РуСтор сам спрашивает согласие и качает в фоне, игра
работает. `immediate` забрал бы экран прямо на старте — это уместно банку, а не
головоломке на пять минут.

### Правила (для будущих сессий)

- **Проверяем один раз за запуск** — `RuStoreUpdateClient.check()` в `App.start()`
  рядом с `RuStoreReviewClient.configure`. Повторная проверка в пределах сессии
  ничего нового не узнает и только жжёт сеть.
- **Ставим только на ТИХОМ переходе между уровнями** —
  `completeIfReady()` в `onNext`, в ветке без интерстишла, перед `loadLevel`.
  Установка перезапускает приложение: посреди уровня это отняло бы разложенные
  ходы (позиция живёт в `savedRun`, а стек undo — нет), а во время рекламы
  оборвало бы показ. Молния там ещё не потрачена — её снимает `loadLevel`.
- **«Скачано» определяем по БАЙТАМ**, а не только по коду статуса: числовые
  константы `InstallStatus` живут в Java, и завязка из JS на их значение молча
  перестанет ставить обновления в день, когда SDK их перенумерует.
- **НЕ переименовывай** `__rustoreUpdateInfo` / `__rustoreUpdateResult` /
  `__rustoreUpdateState` — имена зашиты в Java.
- **НЕ удаляй** `"rustoreUpdateSdk": true` из `.claude/build-config.json`: без
  флага мост не соберётся, а `check()` тихо станет пустышкой — заметить это по
  поведению приложения нельзя.
...
**Двадцать чисел игры крутятся из облака.** Ритм межстраничной и её ГЛАВНЫЙ
ВЫКЛЮЧАТЕЛЬ (`interstitial_enabled`), порог просьбы об оценке, вся экономика
молний вместе со стартовым запасом, кривая сложности целиком, потолок решателя,
ритм обучающей руки и напоминания. Значение приезжает из файла в облаке и
...
**Тип значения задаёт РЕЗЕРВ**: ключи числовые, кроме кривой сложности — она
строка, и жёсткая проверка на `number` отбрасывала бы её молча.

**Два ключа устроены не как остальные.**

`difficulty_curve` — вся кривая строкой `1-3-3-6-(4-4-7-4-4-10)`: до скобки
разгон по уровням, в скобке бесконечный цикл. Разбирает её общий
`remote/remoteConfig.iife.js` (`parseCurve`), тот же код проверяет строку в поле
ввода админки. Негодная запись отбрасывается целиком, и игра идёт по кривой
сборки — `CURVE_PREFIX`/`CURVE_CYCLE` остаются резервом и на случай, когда
клиента нет вовсе (старый WebView). `difficulty_shift` при этом никуда не делся:
он двигает получившуюся кривую целиком, а зажим 1..10 обязателен — пресетов
ровно десять.

`energy_start` — стартовые десять молний. Он правит поле сейва, поэтому
применяется ОДИН раз и **разницей** к значению сборки: `applyStartGrant` в
`[APP]`, флаг `startAdjusted`, `migration[8]`. Присвоение вернуло бы молнию,
потраченную на первый уровень: `loadLevel` зовётся синхронно в конце `start()`,
то есть раньше ответа сети. А читать ключ при создании сейва нельзя вовсе — сейв
рождается в `Persist.getUserId()` для аналитики, ещё до объявления конфига.
