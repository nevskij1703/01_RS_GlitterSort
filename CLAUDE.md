...
**Отладка WebView включена и в release-сборке**, и это не наша настройка:
html2apk кладёт в проект `capacitor.config.json` с
`"webContentsDebuggingEnabled": true`, а Capacitor по этому флагу зовёт
`WebView.setWebContentsDebuggingEnabled(true)` независимо от типа сборки.
Отсюда две вещи: релизную сборку можно проверять тем же отладчиком — и любой, у
кого есть adb, может подключиться к опубликованной игре. Для клиентской игры без
сервера это не новая дыра (сейв в localStorage правится и так), но знать надо.
